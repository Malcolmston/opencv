...
// LoG kernel; the response is pooled into an 8×8 grid of block means and each
// block becomes one bit, set when its mean response exceeds the mean of all
// block responses. The result is a 64-bit (8-byte) [Hash] compared by Hamming
// distance.
//
// Filtering with the LoG operator emphasises edge structure and zero-crossings,
// so the hash keys on shape rather than absolute intensity. This is a
// simplified variant of the multi-orientation 72-bit descriptor. The zero value
// is ready to use; [NewMarrHildrethHash] is provided for symmetry.
...
// Name returns the identifier "marrhildreth".
func (MarrHildrethHash) Name() string { return "marrhildreth" }

// Bits returns 64, the fixed length of a Marr–Hildreth hash.
func (MarrHildrethHash) Bits() int { return 64 }

// Compute returns the 8-byte Marr–Hildreth hash of img.
func (MarrHildrethHash) Compute(img *cv.Mat) Hash {
...
func MarrHildreth(img *cv.Mat) Hash { return MarrHildrethHash{}.Compute(img) }
