...
// CreateBackgroundSubtractorGMG creates a cudalegacy-style GMG subtractor,
// mirroring cv::cuda::createBackgroundSubtractorGMG(initializationFrames,
...
func (b *BackgroundSubtractorGMG) Apply(frame *GpuMat, learningRate float64, stream *Stream) *GpuMat {
	_ = learningRate
	_ = stream
	mask := b.impl.Apply(requireMat(frame, "BackgroundSubtractorGMG.Apply"))
