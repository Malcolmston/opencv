...
// *[cv.Mat]. It exists so that code written against the cudalegacy API — which
// passes frames, masks and label images as GpuMat values and moves them between
// host and device with Upload/Download — compiles and runs unchanged against
// this pure-Go port.
//
...
// Release drops the reference to the wrapped matrix, leaving the GpuMat empty.
// It mirrors cv::cuda::GpuMat::release; there is no device memory to free.
func (g *GpuMat) Release() {
	g.Mat = nil
}

// requireMat extracts a non-empty host matrix from g, panicking with a
// package-tagged message that names the calling operation when g is nil or
// empty. It centralises the precondition shared by every algorithm entry point.
func requireMat(g *GpuMat, op string) *cv.Mat {
	if g == nil || g.Mat == nil || g.Mat.Empty() {
		panic("cudalegacy: " + op + " given a nil or empty GpuMat")
	}
	return g.Mat
}
